...
mcp:
  servers:
    - github # shorthand for a known preset, resolved to a real launch command
    - name: local-fs
...
| `mcp.servers`                  | `(string \| { name, command, args, env })[]`    | `[]`                  | MCP servers to connect to. A bare string must be a known preset (`github`, `postgres`, `playwright`, `filesystem`); otherwise give the full launch command. See [architecture.md](architecture.md#mcp-model-context-protocol-support). |
